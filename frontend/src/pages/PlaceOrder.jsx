    ...
    <div className="flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t">
      ...
      <div className="flex flex-col gap-4 w-full sm:max-w-[480px]">
        ...
        <div className="flex gap-3">
          ...
          <input type="text" className="border border-gray-300 rounded py-1.5 px-3.5 w-full" placeholder="Street/Locality"/>
        <div className="flex gap-3">
        ...
        <div className="mt-12">
          ...
          <div className="flex gap-3 flex-col lg:flex-row">
            <div onClick={()=> setMethod('stripe')} className="flex items-center gap-3 border p-2 px-3 cursor-pointer ">
              ...
              <img src={assets.stripe_logo} alt="payment" className="h-5 mx-4" />
            </div>
            <div onClick={()=> setMethod('razorpay')} className="flex items-center gap-3 border p-2 px-3 cursor-pointer ">
              <p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'razorpay' ? 'bg-green-400' : ''}`}></p>
              <img src={assets.razorpay_logo} alt="payment" className="h-5 mx-4" />
            </div>
          ...
          <div className="w-full text-end mt-8">
            <button onClick={()=> navigate('/orders')} className="bg-black text-white px-16 py-3 text-sm hover:bg-gray-800 transition">PLACE ORDER</button>
          </div>
        </div>
      </div>
    </div>
